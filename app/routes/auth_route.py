...
def register_user():
    ...
    new_user = User(email=email, username=username, password=hashed_password.decode('utf-8'),created_date =datetime.utcnow() )
...
def signin_user():
    ...
    if not user or not bcryptt.check_password_hash(user.password, password):
